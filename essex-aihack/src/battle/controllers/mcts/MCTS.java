package battle.controllers.mcts;

import battle.controllers.heuristic.GameEvaluator;
import framework.core.Controller;
import framework.core.Game;
import framework.core.Waypoint;

import battle.controllers.heuristic.TSPGraphPhysicsEst;
import battle.controllers.heuristic.graph.Path;
...
public class MCTS
{
     ...
     * Graph for the paths
     */
    public battle.controllers.heuristic.graph.Graph m_graph;
    ...
    {
        m_rnd = new Random();
        m_graph = new battle.controllers.heuristic.graph.Graph(a_game);
        ...
        m_tspGraph = new battle.controllers.heuristic.TSPGraphPhysicsEst(a_game, m_graph); //new TSPGraphSight(a_game, m_graph);
...
//		if(values.length>0)
    ...
    private Vector2d getVisiblePointInPath(Game a_game,battle.controllers.heuristic.graph.Path a_p)
    {
        ...
        {
            battle.controllers.heuristic.graph.Node n = m_graph.getNode(a_p.m_points.get(positionToCheck));
            interPos.x = n.x();
            interPos.y = n.y();
    ...
    private Vector2d getBestTargetPointInPath(Game a_game, Waypoint a_closestW, battle.controllers.heuristic.graph.Path a_p)
    {
        ...
        if(interPos == null)
        {
            if(a_p.m_points.size()>1)
            {
                ...
                battle.controllers.heuristic.graph.Node n = m_graph.getNode(which);
    ...
    private battle.controllers.heuristic.graph.Path getPathToWaypoint(Game a_game, Waypoint a_closestW)
    {
        battle.controllers.heuristic.graph.Node shipNode = m_graph.getClosestNodeTo(a_game.getShip().s.x, a_game.getShip().s.y);
        battle.controllers.heuristic.graph.Node waypointNode = m_graph.getClosestNodeTo(a_closestW.s.x, a_closestW.s.y);
