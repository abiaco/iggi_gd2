package battle.controllers.mcts.graph;

import framework.core.Game;
import utilities.ARCCOS;
    ...
    public SightPath(Path a_p, Graph a_graph, Game a_game)
    {
        ...
        if(midPoints.size() > 0)
        {
            ...
            /*if(midPoints.size() == 1)
            {
                ...
                Node dest = a_graph.getNode(p.m_points.get(p.m_points.size()-1)); //dest
                Vector2d b = new Vector2d(dest.x() - n0.x(), dest.y() - n0.y());
                b.normalise();
                last = b.copy();
                angleSum += getAngle(a,b);
            ...
            }else*/
            {
                for(int i = 1; i<=midPoints.size(); ++i)
                {
                    ...
                    if(b.mag() > 0)
                    {
                        ...
                        angleSum += getAngle(a,b);// a.angle(b);
                        a=b;
                    }
                }
            }
