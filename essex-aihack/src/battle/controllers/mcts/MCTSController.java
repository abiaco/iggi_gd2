        ...
        if(bestRoute != null)
        {
            for(int i = 0; i < bestRoute.length; ++i)
            {
                ...
                if(i>0)
                {
                    ...
                    battle.controllers.heuristic.graph.Node org = m_mcts.m_graph.getClosestNodeTo(m_waypointLocations[waypointLastIndex].x, m_waypointLocations[waypointLastIndex].y);
                    battle.controllers.heuristic.graph.Node dest = m_mcts.m_graph.getClosestNodeTo(m_waypointLocations[waypointIndex].x, m_waypointLocations[waypointIndex].y);
                    battle.controllers.heuristic.graph.Path p = m_mcts.m_graph.getPath(org.id(), dest.id());
                    if(p != null) for(int k = 0; k < p.m_points.size()-1; ++k)
                    {
                        battle.controllers.heuristic.graph.Node thisNode = m_mcts.m_graph.getNode(p.m_points.get(k));
                        battle.controllers.heuristic.graph.Node nextNode = m_mcts.m_graph.getNode(p.m_points.get(k+1));
                        //a_gr.drawLine(thisNode.x(), thisNode.y(), nextNode.x(),nextNode.y());
